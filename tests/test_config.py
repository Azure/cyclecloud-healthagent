...
class TestKmsgConfig:
    ...
    @pytest.mark.parametrize("eval_type", set(EvalType) - {EvalType.GE})
    def test_unsupported_eval_rejected(self, eval_type):
        """Kmsg's cumulative count only supports at-least-N semantics."""
        with pytest.raises(ValidationError):
            KmsgPatternCheck(pattern="x", eval=eval_type, error=1)
