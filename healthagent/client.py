...
def print_checks_table(response, phase="all"):
    """Format list_checks response as a table, optionally filtered by phase."""
    rows = []
    for module, checks in response.items():
        for name, info in checks.items():
            categories = info.get("category", [])
            if phase != "all" and phase not in categories:
                continue
...
def main():
    ...
    group.add_argument(
        "-l", "--list-checks", metavar="PHASE", nargs="?", const="all",
        choices=["all", "epilog", "prolog"],
        help="List available checks for a phase (default: all). Example: health -l epilog"
    )

    parser.add_argument(
        "-c", "--check", action="append", nargs="+", metavar="NAME",
        help="Run an prolog/epilog check by name, with optional key=value args. "
             "Repeatable. Example: -c MemoryAllocationTest gpu_id=0,1 -c ActiveDiagnosticChecks"
    )
    ...
    if args.list_checks:
        command = {"command": "list_checks", "phase": "all"}
        response = get_response(command=command, timeout=10)
        if not response:
            sys.exit(-1)
        print_checks_table(response, phase=args.list_checks)
    elif args.epilog:
