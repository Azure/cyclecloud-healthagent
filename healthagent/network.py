...
class NetworkInterface:
    ...
    device: str = None          # resolved sysfs path
...
class NetworkHealthChecks(HealthModule):
    ...
    async def run_network_checks(self):
        ...
        for ni in interfaces:
            ...
            # Build a flat dict of field_name -> current_value for evaluation
            values = {
                "operstate": ni.operstate,
                "carrier_down_count": ni.carrier_down_count,
            }

            # Add IB-specific fields if present
            if ni.ib_device:
                for port_num, port in ni.ib_device.ports.items():
                    values["state"] = port.state
                    values["phys_state"] = port.phys_state
                    values["link_downed"] = port.link_downed
                    values["link_error_recovery"] = port.link_error_recovery
            ...
            for field, check in checks.items():
                ...
                for level in ("error", "warning"):
                    ...
                    if thresh is None:
                        continue
                    triggered, evaluated = evaluate(check["eval"], value, thresh)
                    if triggered:
                        msg = check.get("msg", f" {field}={value} (threshold: {level} {check['eval']} {thresh})")
                        if level == "error":
                            custom_fields.setdefault(ni.name, {}).setdefault("errors", []).append(msg)
                            details.append(f"ERROR: {ni.name} - {msg}")
                            report.escalate(HealthStatus.ERROR)
                        elif level == "warning":
                            custom_fields.setdefault(ni.name, {}).setdefault("warnings", []).append(msg)
                            details.append(f"WARNING: {ni.name} - {msg}")
