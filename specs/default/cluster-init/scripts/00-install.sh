...
if [ -f "$HEALTHAGENT_DIR/.install" ]; then
    # Parse the version string instead of sourcing, to avoid executing the file as root.
    HEALTHAGENT_INSTALLED_VERSION="$(sed -n 's/^HEALTHAGENT_INSTALLED_VERSION=//p' "$HEALTHAGENT_DIR/.install")"
