...
usage() {
    cat <<EOF
Usage: $0 <path-to-healthagent-package.tar.gz>

Sets up HealthAgent on this machine (Python venv, DCGM, and systemd service).

Arguments:
  <path-to-healthagent-package.tar.gz>
        Path to the HealthAgent package blob (a .tar.gz file) to install.
        This is required.
...
Example:
  $0 /path/to/healthagent-1.2.3.tar.gz
EOF
}
